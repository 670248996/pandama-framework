...
public class CustomWebSocketConfigurer implements WebSocketConfigurer {
    ...
    public void registerWebSocketHandlers(@NonNull WebSocketHandlerRegistry registry) {
        ...
        handlers.forEach((beanName, handler) -> {
            WebSocketEndpoint annotation = handler.getClass().getAnnotation(WebSocketEndpoint.class);
            if (annotation == null) {
                throw new RuntimeException(handler.getClass().getSimpleName() +  "实现了WebSocketHandler接口，但是类上没有标注@WebSocketEndpoint注解");
            }
