...
import com.pandama.top.core.utils.FileMD5Utils;
import com.pandama.top.core.utils.FileUtils;
import com.pandama.top.core.utils.SpringContextUtils;
import com.pandama.top.file.biz.constants.FileConstant;
import com.pandama.top.file.biz.file.utils.FilePathUtils;
...
public abstract class SliceUploadTemplate implements SliceUploadStrategy {
    ...
    private boolean setUploadProgress2Redis(FileUploadDTO param, String uploadDirPath, String fileName, File confFile, byte isComplete) {
        ...
        if (isComplete == Byte.MAX_VALUE) {
            redisUtil.hPut(FileConstant.FILE_UPLOAD_STATUS, param.getMd5(), "true");
            redisUtil.delete(FileConstant.FILE_MD5_KEY + param.getMd5());
            confFile.delete();
            return true;
        } else {
            if (!redisUtil.hExists(FileConstant.FILE_UPLOAD_STATUS, param.getMd5()).orElse(Boolean.FALSE)) {
                redisUtil.hPut(FileConstant.FILE_UPLOAD_STATUS, param.getMd5(), "false");
                redisUtil.set(FileConstant.FILE_MD5_KEY + param.getMd5(), uploadDirPath + File.separator + fileName + ".conf");
            }
            return false;
        }
    }
