...
```
由于用户中心模块负责提供全局的用户信息、系统基础信息等实现，故在引入gientech-log模块时重新实现了ILogRecordService接口，
...
可重写gientech-log-core模块中的ILogRecordService方法，将Bean交由Spring容器管理，并标注@Primary以覆盖其原实现方法即可。
```
...
public class LogConfiguration {
    ...
    @Bean
    @Primary
    public ILogRecordService logService() {
        return (logDTO) -> {
