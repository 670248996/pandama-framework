...
import com.pandama.top.user.biz.mapper.DeptMapper;
...
public class DeptServiceImpl extends ServiceImpl<DeptMapper, SysDept> implements DeptService {
    ...
    @Override
    public DeptCreateResultVO create(DeptCreateDTO dto) {
        ...
        try {
            if (lock) {
                ...
                // 将传参字段转换赋值成部门实体属性
                SysDept sysDepartment = BeanConvertUtils.convert(dto, SysDept::new, (s, t) -> {
                ...
                // 封装返回内容
                DeptCreateResultVO resultVO = new DeptCreateResultVO();
                resultVO.setId(sysDepartment.getId());
                return resultVO;
            } else {
    ...
    public void delete(List<Long> departmentIds) {
        // 获取目标逻辑删除的部门
        List<SysDept> deptList = departmentMapper.getDeptListByParentIds(departmentIds, true);
    ...
    @Override
    public List<DeptSearchResultVO> getListByEnterpriseId(Long enterpriseId) {
        List<SysDept> list = departmentMapper.getDeptListByEnterpriseId(enterpriseId);
        return (List<DeptSearchResultVO>) BeanConvertUtils.convertCollection(list, DeptSearchResultVO::new).orElse(new ArrayList<>());
    }

    @Override
    public List<DeptTreeVO> getTreeByEnterpriseId(Long enterpriseId) {
        // 根据企业/机构id获取部门列表
        List<SysDept> deptList = departmentMapper.getDeptListByEnterpriseId(enterpriseId);
        // 返回树状列表
        return TreeUtils.listToTree(deptList, DeptTreeVO::new);
    }

    @Override
    public List<DeptTreeVO> getTreeByDepartmentId(Long deptId) {
