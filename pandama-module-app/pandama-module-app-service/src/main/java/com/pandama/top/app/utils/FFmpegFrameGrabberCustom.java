 ...
 **/
@Slf4j
public class FFmpegFrameGrabberCustom extends FrameGrabber {

    /**
     * 抽帧ti
     */
    private Long grabPerFrameTimeout = 3000L;
    ...
    public static FFmpegFrameGrabberCustom createDefault(File deviceFile) {
        return new FFmpegFrameGrabberCustom(deviceFile);
    }

    public static FFmpegFrameGrabberCustom createDefault(String devicePath) {
        return new FFmpegFrameGrabberCustom(devicePath);
    }

    public static FFmpegFrameGrabberCustom createDefault(int deviceNumber) throws Exception {
        throw new Exception(FFmpegFrameGrabberCustom.class + " does not support device numbers.");
    }
...
//                av_register_all();
                ...
                if (t instanceof Exception) {
                    ...
                    throw loadingException = new Exception("Failed to load " + FFmpegFrameGrabberCustom.class, t);
                }
            }
        }
    }
...
//    }
    ...
    public FFmpegFrameGrabberCustom(File file) {
        this(file.getAbsolutePath());
    }

    public FFmpegFrameGrabberCustom(String filename) {
    ...
    /**
     * Calls {@code FFmpegFrameGrabberCustom(inputStream, Integer.MAX_VALUE - 8)}
     * so that the whole input stream is seekable.
     */
    public FFmpegFrameGrabberCustom(InputStream inputStream) {
     ...
     */
    public FFmpegFrameGrabberCustom(InputStream inputStream, int maximumSize) {
    ...
    public void setVideoFrameNumber(int frameNumber) throws Exception {
        ...
        if (hasVideo()) {
            setVideoTimestamp((long) Math.floor(1000000L * frameNumber / getFrameRate()));
        } else {
    ...
    public void setAudioFrameNumber(int frameNumber) throws Exception {
        // best guess, AVSEEK_FLAG_FRAME has not been implemented in FFmpeg...
        if (hasAudio()) {
            setAudioTimestamp((long) Math.floor(1000000L * frameNumber / getAudioFrameRate()));
        }
...
//                }
    ...
    public synchronized Frame grabFrame(boolean doAudio, boolean doVideo, boolean doProcessing, boolean keyFrames, boolean doData) throws Exception {
        ...
        try (PointerScope scope = new PointerScope()) {
            ...
            while (!done) {
                if (System.currentTimeMillis() - statTime > getGrabPerFrameTimeout()) {
                    throw new Exception("没有获取到新帧");
                }
    ...
    public Long getGrabPerFrameTimeout() {
        return grabPerFrameTimeout;
    }

    public void setGrabPerFrameTimeout(Long grabPerFrameTimeout) {
        this.grabPerFrameTimeout = grabPerFrameTimeout;
    }
}
