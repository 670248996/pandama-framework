...
/**
 * 基于FFmpeg内核来编解码音视频信息；
 * 使用前需手动在运行环境中安装FFmpeg运行程序，然后正确设置FFmpeg运行路径后MediaUtils.java才能正常调用到FFmpeg程序去处理音视频；
 * Author: dreamer-1
 * version: 1.0
 *
 * @author 王强
 * @date 2025-01-02 22:31:11
 */
@Slf4j
public class MediaUtils {
     ...
     * （Time类构造参数的单位:ms）
     */
    private static final Time DEFAULT_TIME = new Time(10 * 1000);
    /**
     * 视频帧抽取的默认宽度值，单位：px
     */
    ...
    public static String getMetaInfoFromFFmpeg(File inputFile) {
        ...
        String executeResult = MediaUtils.executeCommand(commond);
