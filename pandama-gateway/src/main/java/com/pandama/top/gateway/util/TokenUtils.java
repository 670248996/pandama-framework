...
import com.pandama.top.gateway.bean.UserGrantedAuthority;
...
public class TokenUtils implements Serializable {
    ...
    public static User validationToken(String token) {
        try {
            ...
            if (user.getUriCodeList() != null) {
                List<UserGrantedAuthority> uris = user.getUriCodeList().stream()
                        .map(UserGrantedAuthority::new).collect(Collectors.toList());
                user.setUris(uris);
            }
            return user;
        } catch (Exception e) {
